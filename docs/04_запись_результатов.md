...
`TestItWriter` не является классом, который вы напрямую используете в своих тестах. Он создается внутри `TestItReporter`, как мы видели в [прошлой главе](03_репортер.md).
...
*   **Перед всеми тестами в классе (`beforeSpec`):** `TestItWriter.onBeforeAll` -> Запускает контейнеры для группировки тестов ([AdapterManager](01_менеджер_адаптера.md)'s `startMainContainer`, `startClassContainer`).
*   **После всех тестов в классе (`afterSpec`):** `TestItWriter.onAfterAll` -> Завершает контейнеры ([AdapterManager](01_менеджер_адаптера.md)'s `stopClassContainer`, `stopMainContainer`).
...
Диаграмма показывает, как `TestItWriter` принимает событие от `TestItReporter`, определяет, что это тест, и передает управление `TestService`. `TestService`, в свою очередь, использует [AdapterManager](01_менеджер_адаптера.md) для фактического управления жизненным циклом теста и его отправки.
...
2.  Он передает этим сервисам ссылку на [AdapterManager](01_менеджер_адаптера.md) и общие структуры данных (`uuids`).
...
*   **[Репортер Test IT (TestItReporter)](03_репортер.md):** Получает от него необработанные события Kotest.
...
*   **[Менеджер Адаптера (AdapterManager)](01_менеджер_адаптера.md):** Не напрямую, а через сервисы. `TestItWriter` передает ссылку на `AdapterManager` в конструкторы сервисов.

## Заключение

Мы разобрались с ролью `TestItWriter` — это важный **посредник и маршрутизатор** в архитектуре адаптера. Он принимает "сырые" сигналы о событиях жизненного цикла тестов от [TestItReporter](03_репортер.md), анализирует их и передает команды конкретным "исполнителям" — сервисам (`TestService`, `StepService`, `FixtureService`). Эти сервисы уже выполняют основную работу по взаимодействию с [AdapterManager](01_менеджер_адаптера.md) для управления результатами тестов и их отправки в Test IT.
