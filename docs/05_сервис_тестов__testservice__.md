...
В [прошлой главе](04_запись_результатов.md) мы познакомились с [Записью Результатов (TestItWriter)](04_запись_результатов.md), нашим "ассистентом режиссера", который получает сигналы от репортера и передает команды дальше. Он решает, *кто* должен выполнить работу: сервис для тестов, сервис для шагов или сервис для настроек (фикстур).
...
Представьте снова нашу съемочную площадку. [TestItWriter](04_запись_результатов.md) — ассистент режиссера, получает команду "Начинаем снимать сцену 'Проверка логина'!". Он знает, что это *основная* сцена, а не подготовительный кадр или короткий шаг. Поэтому он обращается к **главному оператору (`TestService`)**, который отвечает именно за съемку таких ключевых сцен.
...
1.  **Зарегистрировать начало:** Когда [TestItWriter](04_запись_результатов.md) говорит "Начинаем!", `TestService` сообщает [Менеджеру Адаптера (AdapterManager)](01_менеджер_адаптера.md), что тест стартовал.
2.  **Зафиксировать результат:** Когда тест завершается (успешно, с ошибкой или был пропущен), [TestItWriter](04_запись_результатов.md) сообщает об этом `TestService`. `TestService`, в свою очередь, передает финальный статус и возможные ошибки [AdapterManager](01_менеджер_адаптера.md).
3.  **Связать информацию:** Он также помогает правильно оформить всю информацию о тесте (имя, класс, уникальный ID), чтобы [AdapterManager](01_менеджер_адаптера.md) мог корректно сохранить и отправить ее.
...
Вы, как пользователь, **никогда не вызываете `TestService` напрямую**. Его вызывает [TestItWriter](04_запись_результатов.md). Давайте вспомним код из `TestItWriter` (из [Главы 4](04_запись_результатов.md)), где это происходит:
...
Когда `TestItWriter` вызывает методы `TestService`, последний не делает всю работу сам. Он, в свою очередь, дает команды главному диспетчеру — [Менеджеру Адаптера (AdapterManager)](01_менеджер_адаптера.md).
...
*   **[Менеджер Адаптера (AdapterManager)](01_менеджер_адаптера.md):** Основной компонент, которому `TestService` делегирует выполнение команд по управлению жизненным циклом теста.
*   **[Запись Результатов (TestItWriter)](04_запись_результатов.md):** Компонент, который вызывает методы `TestService` в ответ на события от `TestItReporter`.
