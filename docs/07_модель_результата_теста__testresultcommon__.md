...
*   `automaticCreationTestCases: Boolean` ✨
    ...
    *   **Зачем:** Берется из [конфигурации](02_конфигурация_адаптера.md), чтобы управлять созданием тестов.
...
4.  **Завершение:** Когда тест заканчивается, [TestItWriter](04_запись_результатов.md) снова вызывает [TestService](05_сервис_тестов__testservice__.md), который передает результат (успех/провал/игнор) и ошибку (если есть) в `AdapterManager` (`updateTestCase`, `stopTestCase`). `AdapterManager` делает финальные штрихи:
...
5.  **Отправка:** Наконец, полностью заполненный `TestResultCommon` передается компоненту [Запись Результатов (например, HttpWriter)](08_запись_результатов.md), который преобразует его (`Converter`) в специальный формат для API Test IT и отправляет по сети с помощью [API Клиента TMS (TmsApiClient)](09_api_клиент_tms__tmsapiclient__.md).
...
*   **[Менеджер Адаптера (AdapterManager)](01_менеджер_адаптера.md):** Управляет жизненным циклом `TestResultCommon`, обновляет его статус, время, применяет контекст.
...
*   **[Запись Результатов по HTTP (HttpWriter)](08_запись_результатов.md) (или другой `Writer`):** Получает финальный `TestResultCommon` для отправки.
...
**Далее:** [Глава 8: Запись Результатов по HTTP (HttpWriter)](08_запись_результатов.md)
